...
import time
import os
from robosuite.controllers.helping_func_class import LinearSchedule,convert_tool_to_joints
import gym
        ...
        for itr in range(self.n_iter):
            ...
            for num_path in range(self.paths):
                ...
                for t_horizon in range(self.horizon):
                    ...
                    if self.space=="enf":
                        param=self.sess.run(self.sy_sampled_ac, feed_dict={self.sy_ob_no: ob[None, :]})
                        ac=convert_tool_to_joints(self.env,param)
                    else:
                    ...
                    if self.exploration.value(itr)>np.random.rand(1)[0]:
                        param =np.expand_dims(np.random.randn(self.action_space),axis=0)
                        if self.space == "enf":
                            ac = convert_tool_to_joints(self.env,param)
                        else:
                            ac=param
