...
from robosuite.models.objects import CylinderObject, PlateWithHoleObject,LemonObject
...
class BaxterRandomPoints(BaxterEnv):
    ...
    def _load_model(self):
        ...
        # Load hole object
        #self.balloon_obj = self.hole.get_collision(name="hole", site=True)


        #for i in range(self.number_of_points):
        quat="0 0 "+str(np.random.rand(1)[0])+" "+str(np.random.rand(1)[0])
        pos=str(np.random.rand(1)[0])+" "+str(np.random.rand(1)[0])
        self.balloon.set("quat", quat)
        self.balloon.set("pos", pos)

        self.model.merge_asset(self.balloon)
