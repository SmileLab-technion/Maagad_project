...
import numpy as np
from robosuite.controllers.helping_func_class import convert_tool_to_joints
...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def __init__(
        ...
        self.action_space = spaces.Box(low=-1, high=1, shape=(self.dof,))
        self.observation_space = spaces.Box(low=-np.inf*np.ones(self.observation_space_shape), high=np.inf*np.ones(self.observation_space_shape))
    ...
    def step(self, action):
        ...
        difr=1e3
        steps=0
        while difr>1e-3 and steps<10:
            error_right,error_left=self._PD_feedback(action)[0],self._PD_feedback(action)[1]
            ...
            error_right_2, error_left_2 = self._PD_feedback(action)[0], self._PD_feedback(action)[1]
            difr=np.abs(error_right_2-error_right)+np.abs(error_left_2-error_left)
            steps+=1
